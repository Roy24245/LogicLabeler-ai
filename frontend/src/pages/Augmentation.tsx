...
import {
  ...
  Alert, LinearProgress, useTheme, alpha,
} from '@mui/material'
...
export default function Augmentation() {
  ...
  const handleOpenOptimize = () => {
    const trimmed = labelInstruction.trim()
  ...
  return (
    <Box>
      <PageHeader
        ...
        title="數據增強"
        subtitle="運用 AI 圖片生成技術擴充數據集，並自動為新樣本標註"
      />

      {isDisabled && <Alert severity="warning" sx={{ mb: 2 }}>數據增強功能已關閉。請前往「系統設定」頁面開啟。</Alert>}
            ...
            <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                {VARIATION_TYPES.map((v) => {
                  const sel = selectedTypes.includes(v.id)
                  return (
                    <Card key={v.id} sx={{
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', px: 1.5, py: 1 }}>
                        <Checkbox checked={sel} disabled={running} size="small" sx={{ mr: 1 }} />
                        <Box>
              ...
              <Card sx={{ p: 2, bgcolor: alpha(theme.palette.secondary.main, 0.04) }}>
                <FormControlLabel
                ...
                {autoLabel && (
                  <Box sx={{ mt: 1.5 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.75 }}>
                      <Typography variant="caption" color="text.secondary">標註指令（選填）</Typography>
                      <Button
                        size="small"
                        variant="tonal"
                        startIcon={<TipsAndUpdatesRoundedIcon />}
                        onClick={handleOpenOptimize}
                        disabled={!labelInstruction.trim() || running}
                      >
                        優化 Prompt
                      </Button>
                    </Box>
                    <TextField fullWidth size="small" placeholder="例如: 檢測圖片中的行人、車輛"
              ...
              <Button variant="contained" size="large" fullWidth startIcon={<PlayArrowRoundedIcon />} onClick={handleRun} disabled={running || !!isDisabled}>
        ...
        <Grid item xs={12} md={7}>
          <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              {jobStatus?.status === 'failed' && <Alert severity="error" sx={{ mt: 2 }}>任務失敗：{jobStatus.error || '未知錯誤'}</Alert>}
            </CardContent>
          </Card>
        </Grid>
      </Grid>
      ...
      <PromptOptimizerDialog
        ...
        onApply={(text) => { setLabelInstruction(text); setOptimizeOpen(false) }}
      />
    </Box>
  )
}
