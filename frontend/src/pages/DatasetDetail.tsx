...
import {
  ...
  CircularProgress, FormControlLabel, Menu,
} from '@mui/material'
...
import {
  ...
  batchDeleteImages, getDatasetStats, autoSplit, batchSplit, updateDataset, convertImagesToJpg,
  type Dataset, type ImageItem, type AnnotationItem, type DatasetStats as StatsType,
} from '../api/client'
...
export default function DatasetDetail() {
  ...
  const classColors = useMemo(() => {
    ...
    for (const a of annotations) {
      if (!m[a.class_name]) { m[a.class_name] = COLORS[ci % COLORS.length]; ci++ }
    }
    return m
  }, [annotations])
  ...
  const refreshAll = useCallback(async () => {
    await Promise.all([loadDataset(), loadImages(), loadStats()])
  }, [loadDataset, loadImages, loadStats])
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      await batchDeleteImages(datasetId, Array.from(selectedIds))
      setSelectedIds(new Set())
      showSnackbar('批量刪除完成', 'success')
      refreshAll()
  ...
  const handleBatchSplit = async (splitVal: string | null) => {
    ...
    try {
      await batchSplit(datasetId, Array.from(selectedIds), splitVal)
      setBatchMenuAnchor(null)
      setSelectedIds(new Set())
      showSnackbar('分割指定完成', 'success')
      refreshAll()
  ...
  const handleAutoSplit = async () => {
    try {
      await autoSplit(datasetId, splitRatios[0] / 100, splitRatios[1] / 100, splitRatios[2] / 100)
      showSnackbar('自動分割完成', 'success')
      refreshAll()
    } catch { showSnackbar('分割失敗', 'error') }
  }

  // ── Annotator: open/close/nav ──
  const openAnnotator = async (img: ImageItem, idx: number) => {
    ...
    try {
      ...
      setAnnotations(data as LocalAnn[])
    } catch { setAnnotations([]) }
  }

  const closeAnnotator = async () => {
    if (dirty) {
      await autoSave()
    }
  ...
  const navigateImage = async (dir: number) => {
    const newIdx = selectedImageIdx + dir
    if (newIdx < 0 || newIdx >= images.length) return
    await openAnnotator(images[newIdx], newIdx)
  }
  ...
  const autoSave = async () => {
    ...
    try {
      const payload = annotations.map(a => ({
        class_name: a.class_name, bbox: a.bbox, confidence: a.confidence, source: a.source,
      }))
      await updateAnnotations(selectedImage.id, payload)
      setDirty(false)
    } catch { /* silent */ }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data } = await updateAnnotations(selectedImage.id, payload)
      setAnnotations(data as LocalAnn[])
      setDirty(false)
      setUndoStack([])
      setRedoStack([])
      showSnackbar('標註已保存', 'success')
    } catch { showSnackbar('保存失敗', 'error') }
  }

  // ── Undo/Redo ──
  const pushUndo = (prev: LocalAnn[]) => {
  ...
  // ── Copy/Paste ──
  const handleCopy = () => {
    if (selectedAnnIdx >= 0 && annotations[selectedAnnIdx]) {
      setClipboard({ ...annotations[selectedAnnIdx] })
      showSnackbar('已複製標註', 'info')
    }
  }
  ...
  const handlePaste = () => {
    ...
    const newAnn: LocalAnn = {
      ...clipboard,
      id: -Date.now(),
      bbox: clipboard.bbox ? { ...clipboard.bbox, x: clipboard.bbox.x + 20, y: clipboard.bbox.y + 20 } : null,
    }
    setAnnotations(anns => [...anns, newAnn])
    setSelectedAnnIdx(annotations.length)
    setToolMode('edit')
  }

  const deleteAnnotation = (idx: number) => {
    pushUndo([...annotations])
    setAnnotations(anns => anns.filter((_, i) => i !== idx))
    setSelectedAnnIdx(-1)
  }
  ...
  const getTransform = () => {
    ...
    return {
      scale: b.scale * zoom,
      offsetX: b.offsetX * zoom + panOffset.x,
      offsetY: b.offsetY * zoom + panOffset.y,
    }
  }
  ...
  const imageToCanvas = (ix: number, iy: number) => {
    const t = getTransform()
  ...
  const drawCanvas = useCallback(() => {
    ...
    // Apply brightness/contrast via filter (with fallback for Safari)
    const needsFilter = brightness !== 100 || contrast !== 100
    if (needsFilter) {
      try { ctx.filter = `brightness(${brightness / 100}) contrast(${contrast / 100})` } catch { /* unsupported */ }
    }
    ...
    annotations.forEach((ann, idx) => {
      if (!ann.bbox) return
      const color = classColors[ann.class_name] || '#fff'
      const pos = imageToCanvas(ann.bbox.x, ann.bbox.y)
      const w = ann.bbox.w * t.scale
      const h = ann.bbox.h * t.scale
      const isSelected = idx === selectedAnnIdx && toolMode === 'edit'

      if (ann.review_status === 'rejected') {
        ctx.setLineDash([6, 4]); ctx.strokeStyle = '#FF3D00'
      } else if (ann.review_status === 'needs_adjustment') {
        ctx.setLineDash([6, 4]); ctx.strokeStyle = '#FF9100'
      } else {
      ...
      ctx.fillStyle = ann.review_status === 'rejected' ? '#FF3D00' : ann.review_status === 'needs_adjustment' ? '#FF9100' : color
      ctx.fillRect(pos.x, pos.y - 20, tw + 8, 20)
      ctx.fillStyle = '#fff'
    ...
    if (drawRect) {
      ...
      ctx.lineWidth = 2
      ctx.setLineDash([4, 4])
      const rx = Math.min(drawRect.x1, drawRect.x2)
      const ry = Math.min(drawRect.y1, drawRect.y2)
      const rw = Math.abs(drawRect.x2 - drawRect.x1)
      const rh = Math.abs(drawRect.y2 - drawRect.y1)
    ...
    // Crosshair in draw mode
    if (toolMode === 'draw' && mouseCanvas) {
      ctx.strokeStyle = 'rgba(124, 77, 255, 0.5)'
      ctx.lineWidth = 1
      ctx.setLineDash([4, 4])
      ctx.beginPath()
      ctx.moveTo(mouseCanvas.x, 0); ctx.lineTo(mouseCanvas.x, canvas.height)
      ctx.moveTo(0, mouseCanvas.y); ctx.lineTo(canvas.width, mouseCanvas.y)
      ctx.stroke()
      ctx.setLineDash([])
    }
  ...
  // Load image — no crossOrigin needed (same-origin); bump imgLoaded to trigger drawCanvas
  useEffect(() => {
    if (!selectedImage) return
    imgRef.current = null
    ...
    image.onload = () => {
      ...
      if (canvas) {
        ...
        baseTransformRef.current = { scale, offsetX, offsetY, imgW: image.width, imgH: image.height }
      }
      setImgLoaded(n => n + 1)
    }
    image.onerror = () => {
  ...
  const findAnnotationAt = (cx: number, cy: number): number => {
    const t = getTransform()
    for (let i = annotations.length - 1; i >= 0; i--) {
      const a = annotations[i]
      if (!a.bbox) continue
      const p = imageToCanvas(a.bbox.x, a.bbox.y)
      const w = a.bbox.w * t.scale
      const h = a.bbox.h * t.scale
      if (cx >= p.x && cx <= p.x + w && cy >= p.y && cy <= p.y + h) return i
    }
    return -1
  }

  const findHandle = (cx: number, cy: number, annIdx: number): HandlePos | null => {
    ...
    const w = a.bbox.w * t.scale
    const h = a.bbox.h * t.scale
    const handles = getHandlePositions(p.x, p.y, w, h)
    const names: HandlePos[] = ['tl', 'tr', 'bl', 'br', 'tm', 'bm', 'ml', 'mr']
    for (let i = 0; i < handles.length; i++) {
      if (Math.abs(cx - handles[i].x) <= HANDLE_SIZE && Math.abs(cy - handles[i].y) <= HANDLE_SIZE) return names[i]
    }
    return null
  }
  ...
  const onMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (e.button === 1) {
      dragRef.current = { action: 'pan', startX: cx, startY: cy, origBbox: { x: 0, y: 0, w: 0, h: 0 }, handle: null, annIdx: -1, origPan: { ...panOffset } }
      return
    }

    if (toolMode === 'draw') {
      dragRef.current = { action: 'draw', startX: cx, startY: cy, origBbox: { x: 0, y: 0, w: 0, h: 0 }, handle: null, annIdx: -1 }
      setDrawRect({ x1: cx, y1: cy, x2: cx, y2: cy })
      return
    }

    if (toolMode === 'edit') {
      if (selectedAnnIdx >= 0) {
        const handle = findHandle(cx, cy, selectedAnnIdx)
        if (handle) {
          const a = annotations[selectedAnnIdx]
          pushUndo([...annotations])
          dragRef.current = { action: 'resize', startX: cx, startY: cy, origBbox: { ...a.bbox! }, handle, annIdx: selectedAnnIdx }
          return
        }
      }
      ...
      if (idx >= 0) {
        ...
        pushUndo([...annotations])
        dragRef.current = { action: 'move', startX: cx, startY: cy, origBbox: { ...a.bbox! }, handle: null, annIdx: idx }
      } else {
        setSelectedAnnIdx(-1)
      }
    }
  ...
  const onMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    const { cx, cy } = getCanvasPos(e)
    if (toolMode === 'draw') setMouseCanvas({ x: cx, y: cy })
    ...
    if (d.action === 'pan' && d.origPan) {
      ...
      const dx = (e.clientX - rect.left) * scaleX - d.startX
      const dy = (e.clientY - rect.top) * scaleX - d.startY
      setPanOffset({ x: d.origPan.x + dx, y: d.origPan.y + dy })
      return
    }

    if (d.action === 'draw') {
      setDrawRect({ x1: d.startX, y1: d.startY, x2: cx, y2: cy })
      return
    }
    ...
    if (d.action === 'move') {
      ...
      const b = baseTransformRef.current
      setAnnotations(anns => anns.map((a, i) => {
        if (i !== d.annIdx || !a.bbox) return a
        return { ...a, bbox: { ...d.origBbox, x: clamp(d.origBbox.x + dx, 0, b.imgW - d.origBbox.w), y: clamp(d.origBbox.y + dy, 0, b.imgH - d.origBbox.h) } }
      }))
      return
    }

    if (d.action === 'resize' && d.handle) {
      const dx = (cx - d.startX) / t.scale
      const dy = (cy - d.startY) / t.scale
      const b = baseTransformRef.current
      setAnnotations(anns => anns.map((a, i) => {
        if (i !== d.annIdx || !a.bbox) return a
        return { ...a, bbox: resizeBox(d.origBbox, d.handle!, dx, dy, b.imgW, b.imgH) }
      }))
    }
  }
  ...
  const onMouseUp = () => {
    ...
    if (d.action === 'draw' && drawRect) {
      const b = baseTransformRef.current
      const p1 = canvasToImage(Math.min(drawRect.x1, drawRect.x2), Math.min(drawRect.y1, drawRect.y2))
      const p2 = canvasToImage(Math.max(drawRect.x1, drawRect.x2), Math.max(drawRect.y1, drawRect.y2))
      const w = p2.x - p1.x
      const h = p2.y - p1.y
      if (w > 3 && h > 3) {
        ...
        pushUndo([...annotations])
        const newAnn: LocalAnn = {
          id: -Date.now(), class_name: cls,
          bbox: { x: clamp(p1.x, 0, b.imgW), y: clamp(p1.y, 0, b.imgH), w: Math.min(w, b.imgW - p1.x), h: Math.min(h, b.imgH - p1.y) },
          confidence: null, source: 'manual', review_status: null, review_comment: null,
        }
        setAnnotations(anns => [...anns, newAnn])
        setSelectedAnnIdx(annotations.length)
        setToolMode('edit')
  ...
  const onWheel = (e: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    const newZoom = clamp(zoom * factor, 0.2, 10)
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      switch (true) {
        ...
        case e.key === 'Delete' || e.key === 'Backspace':
          if (selectedAnnIdx >= 0) { deleteAnnotation(selectedAnnIdx); e.preventDefault() }
          break
  ...
  const updateAnnotationClass = (idx: number, newClass: string) => {
    pushUndo([...annotations])
    setAnnotations(anns => anns.map((a, i) => i === idx ? { ...a, class_name: newClass } : a))
  }
    ...
    <Box>
      ...
      {tab === 0 && (
        <Box {...getRootProps()}>
          {/* Drop overlay */}
          {isDragActive && (
            <Box sx={{ position: 'fixed', inset: 0, zIndex: 9999, bgcolor: 'rgba(25,118,210,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <TextField
              size="small" placeholder="搜索檔名..." value={searchText}
              onChange={(e) => { setSearchText(e.target.value); setPage(1) }}
              InputProps={{ startAdornment: <SearchIcon fontSize="small" sx={{ mr: 0.5, color: 'text.secondary' }} /> }}
              sx={{ width: 200 }}
            />
            <Box sx={{ flex: 1 }} />
            <Typography variant="body2" color="text.secondary">{totalImages} 張圖片</Typography>
          </Box>

          {/* Batch operations bar */}
          {selectedIds.size > 0 && (
            <Box sx={{ display: 'flex', gap: 1, mb: 2, alignItems: 'center', p: 1, bgcolor: 'action.hover', borderRadius: 1 }}>
              <Typography variant="body2">已選 {selectedIds.size} 張</Typography>
              <Button size="small" color="error" startIcon={<DeleteIcon />} onClick={handleBatchDelete}>批量刪除</Button>
          ...
          {loading ? (
            ...
            <>
              <Box sx={{ display: 'flex', gap: 0.5, mb: 1 }}>
                <Tooltip title="全選/取消全選">
                  <IconButton size="small" onClick={selectAll}>
                    <SelectAllIcon fontSize="small" color={selectedIds.size === images.length && images.length > 0 ? 'primary' : 'inherit'} />
                  </IconButton>
                </Tooltip>
              </Box>
                  ...
                  <Grid item xs={6} sm={4} md={3} lg={2} key={img.id}>
                    <Card
                      sx={{
                        ...
                        border: selectedIds.has(img.id) ? '2px solid' : '1px solid',
                        borderColor: selectedIds.has(img.id) ? 'primary.main' : 'divider',
                        '&:hover': { transform: 'scale(1.02)' },
                      }}
                      onClick={() => openAnnotator(img, idx)}
                    >
                      <CardMedia component="img" height={140} image={img.url} alt={img.filename} sx={{ objectFit: 'cover' }} />
                      <Box sx={{ p: 0.75, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <IconButton size="small" onClick={(e) => { e.stopPropagation(); deleteImage(img.id).then(() => refreshAll()) }} color="error">
                          <DeleteIcon fontSize="small" />
                        </IconButton>
                      </Box>
                      {/* Status chips */}
                      <Chip
                        label={img.annotation_count > 0 ? `${img.annotation_count}` : '未標註'}
                        size="small"
                        color={img.annotation_count > 0 ? 'success' : 'default'}
                        sx={{ position: 'absolute', top: 4, left: 4, fontSize: 10, height: 20 }}
                      />
                      {img.split && SPLIT_CHIP[img.split] && (
                        <Chip
                          label={SPLIT_CHIP[img.split].label}
                          size="small"
                          color={SPLIT_CHIP[img.split].color}
                          sx={{ position: 'absolute', top: 4, right: 4, fontSize: 10, height: 20 }}
                        />
                      )}
                      ...
                      <Checkbox
                        ...
                        sx={{ position: 'absolute', bottom: 28, right: 0, p: 0.5 }}
                      />
                    </Card>
                  </Grid>
                ))}
              ...
              {images.length === 0 && !loading && (
                <Box sx={{ textAlign: 'center', py: 8 }}>
                  <AddPhotoAlternateIcon sx={{ fontSize: 64, color: 'text.secondary' }} />
                  <Typography color="text.secondary" sx={{ mt: 2 }}>拖拽圖片到此處或點擊上傳按鈕</Typography>
                </Box>
              )}
            </>
          )}
      ...
      {/* ─── Tab 1: Stats ─── */}
      {tab === 1 && (
        stats ? <DatasetStatsPanel stats={stats} /> : <Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}><CircularProgress /></Box>
      )}
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
              ...
              {stats && (
                <Box sx={{ mb: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  {Object.entries(stats.split_distribution).map(([k, v]) => (
                    <Chip key={k} label={`${k === 'unassigned' ? '未分配' : k}: ${v}`}
                      color={SPLIT_CHIP[k]?.color || 'default'} size="small" variant="outlined" />
                  ))}
                </Box>
              )}
              ...
              <Box sx={{ px: 2 }}>
                ...
                <Slider
                  ...
                  onChange={(_, v) => {
                    ...
                    const val = Math.min(splitRatios[1], remaining - 5)
                    setSplitRatios([train, Math.max(5, val), remaining - Math.max(5, val)])
                  }}
                  sx={{ mb: 1 }}
                />
                <Slider
                  value={splitRatios[1]}
                  min={5} max={100 - splitRatios[0] - 5} step={5}
                  onChange={(_, v) => {
                    const val = v as number
                    setSplitRatios([splitRatios[0], val, 100 - splitRatios[0] - val])
                  }}
                  color="secondary"
                />
              </Box>
              <Button variant="contained" onClick={handleAutoSplit} sx={{ mt: 2 }} fullWidth>
                執行自動分割
              </Button>
            </Card>
            ...
            <Card sx={{ p: 2, mt: 2 }}>
              ...
              <TextField fullWidth label="名稱" size="small" value={dataset.name} sx={{ mb: 2 }}
                onChange={() => {}} disabled
              />
              <TextField fullWidth label="描述" size="small" multiline rows={2} value={dataset.description || ''} sx={{ mb: 2 }}
                onChange={() => {}} disabled
              />
              <Typography variant="body2" color="text.secondary">
                類型: {dataset.task_type} · 創建: {dataset.created_at ? new Date(dataset.created_at).toLocaleDateString() : '-'}
              </Typography>
            </Card>
            ...
            <Card sx={{ p: 2, mt: 2 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                將數據集中所有非 JPG 格式的圖片（PNG、BMP、WebP、TIFF 等）統一轉換為 JPEG 格式。新上傳和導入的圖片會自動轉換。
              </Typography>
              <Button
                variant="outlined"
                fullWidth
                onClick={async () => {
                  try {
                    const res = await convertImagesToJpg(datasetId)
                    alert(`轉換完成，共轉換 ${res.data.converted} 張圖片`)
                    refreshAll()
                  } catch {
                    alert('轉換失敗')
                  }
      ...
      <Dialog open={!!selectedImage} onClose={closeAnnotator} maxWidth="xl" fullWidth PaperProps={{ sx: { height: '90vh' } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1, px: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <IconButton onClick={() => navigateImage(-1)} disabled={selectedImageIdx <= 0} size="small">
              <NavigateBeforeIcon />
            </IconButton>
            <Typography variant="subtitle1" noWrap sx={{ maxWidth: 300 }}>
              {selectedImage?.filename} ({selectedImageIdx + 1}/{images.length})
            </Typography>
            <IconButton onClick={() => navigateImage(1)} disabled={selectedImageIdx >= images.length - 1} size="small">
              <NavigateNextIcon />
            </IconButton>
          </Box>
          <IconButton onClick={closeAnnotator}><CloseIcon /></IconButton>
        </DialogTitle>

        <DialogContent sx={{ p: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {/* Toolbar */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 0.75, borderBottom: '1px solid', borderColor: 'divider', flexWrap: 'wrap', flexShrink: 0 }}>
            <ToggleButtonGroup size="small" exclusive value={toolMode} onChange={(_, v) => { if (v) { setToolMode(v); setSelectedAnnIdx(-1) } }}>
              <ToggleButton value="view"><Tooltip title="查看 (V)"><VisibilityIcon fontSize="small" /></Tooltip></ToggleButton>
              <ToggleButton value="draw"><Tooltip title="繪製 (D)"><DrawIcon fontSize="small" /></Tooltip></ToggleButton>
              <ToggleButton value="edit"><Tooltip title="編輯 (E)"><EditIcon fontSize="small" /></Tooltip></ToggleButton>
            </ToggleButtonGroup>

            {toolMode === 'draw' && (
              <Autocomplete
                freeSolo size="small" sx={{ width: 180 }}
                options={allClasses}
                value={drawClassName}
            ...
            <Tooltip title="撤銷 (Ctrl+Z)"><span><IconButton size="small" onClick={handleUndo} disabled={!undoStack.length}><UndoIcon fontSize="small" /></IconButton></span></Tooltip>
            <Tooltip title="重做 (Ctrl+Shift+Z)"><span><IconButton size="small" onClick={handleRedo} disabled={!redoStack.length}><RedoIcon fontSize="small" /></IconButton></span></Tooltip>
            <Tooltip title="複製 (Ctrl+C)"><span><IconButton size="small" onClick={handleCopy} disabled={selectedAnnIdx < 0}><ContentCopyIcon fontSize="small" /></IconButton></span></Tooltip>
            <Tooltip title="貼上 (Ctrl+V)"><span><IconButton size="small" onClick={handlePaste} disabled={!clipboard}><ContentPasteIcon fontSize="small" /></IconButton></span></Tooltip>
            <Tooltip title="回到全景 (Ctrl+0)"><IconButton size="small" onClick={fitView}><FitScreenIcon fontSize="small" /></IconButton></Tooltip>

            <Divider orientation="vertical" flexItem />
            <Tooltip title="亮度/對比度">
              <IconButton size="small" onClick={() => setShowAdjust(!showAdjust)} color={brightness !== 100 || contrast !== 100 ? 'primary' : 'default'}>
            ...
            {showAdjust && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <BrightnessIcon fontSize="small" />
                <Slider value={brightness} min={30} max={200} onChange={(_, v) => setBrightness(v as number)} sx={{ width: 80 }} size="small" />
                <ContrastIcon fontSize="small" />
                <Slider value={contrast} min={30} max={200} onChange={(_, v) => setContrast(v as number)} sx={{ width: 80 }} size="small" />
            ...
            <Typography variant="caption" color="text.secondary">
              {annotations.length} 標註 · {Math.round(zoom * 100)}%
            </Typography>
            <Tooltip title="保存 (Ctrl+S)">
              <span>
                <Button size="small" variant="contained" startIcon={<SaveIcon />} onClick={handleSave} disabled={!dirty}>
                  保存{dirty ? ' *' : ''}
                </Button>
              </span>
            </Tooltip>
          ...
          <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
            {/* Canvas */}
            <Box sx={{ flex: 1, position: 'relative', bgcolor: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
              <canvas
            ...
            <Box sx={{ width: 320, borderLeft: '1px solid', borderColor: 'divider', overflow: 'auto', p: 1.5, flexShrink: 0 }}>
              <Typography variant="subtitle2" gutterBottom>標註列表 ({annotations.length})</Typography>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                {annotations.map((ann, i) => (
                  <Card
                    ...
                    sx={{
                      p: 1, cursor: 'pointer',
                      border: selectedAnnIdx === i ? '2px solid' : '1px solid',
                      borderColor: selectedAnnIdx === i ? 'primary.main' : 'divider',
                    }}
                  ...
                  >
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.25 }}>
                      {selectedAnnIdx === i ? (
                        <Autocomplete
                          freeSolo size="small" sx={{ flex: 1, mr: 1 }}
                          options={allClasses}
                        ...
                        <Chip label={ann.class_name} size="small" sx={{ bgcolor: (classColors[ann.class_name] || '#666') + '33', maxWidth: 150 }} />
                      )}
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.25 }}>
                        {ann.review_status === 'approved' && <Tooltip title="AI 通過"><CheckCircleIcon sx={{ fontSize: 16, color: '#69F0AE' }} /></Tooltip>}
                        {ann.review_status === 'rejected' && <Tooltip title={ann.review_comment || '拒絕'}><CancelIcon sx={{ fontSize: 16, color: '#FF3D00' }} /></Tooltip>}
                        {ann.review_status === 'needs_adjustment' && <Tooltip title={ann.review_comment || '需調整'}><WarningIcon sx={{ fontSize: 16, color: '#FF9100' }} /></Tooltip>}
                        <IconButton size="small" onClick={(e) => { e.stopPropagation(); deleteAnnotation(i) }} color="error">
